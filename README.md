...
上記の `generate-data` が作るのは、小売を想定した**集計済みの日次データ**である。
これとは別に、法人向け販売の**集計前の取引明細**を生成できる。
...
土日祝は受注ゼロ、年度末の予算消化、部署ごとの得意商材、顧客ごとの値引率
といった法人取引の特徴を組み込んでいる（`src/demand_forecast/data/transactions.py`）。

日次の系列データへの集計は取り込み側の責務として分けてあり、
系列の粒度（部署別・商品別・顧客別）を後から選べる。
