...
> [プロジェクト概要（3分）](docs/for-business/01-overview.md) から読んでください。
...
> **先に読んでほしい前提**: 本リポジトリのデータは合成であり、
> 季節性・曜日周期・部署の得意商材は**生成器に定数で書き込んである**。
> モデルがそれを当てるのは当然で、そこに情報は無い。
> どこまでが仕込みで、どこからが実際に分析して分かったことかは
> [`docs/data-understanding.md`](docs/data-understanding.md) に分けて記録している。
...
| **合成データである** | 季節性・曜日周期・部署の傾向は生成器に定数で書き込んであり、モデルがそれを当てても実力の証明にならない。新規獲得・離脱・価格改定・返品はいずれも0件で、その種の分析はできない。→ [データ理解](docs/data-understanding.md) |
...
- [非エンジニア向けの説明](docs/for-business/) — **数式なしの入門**（概要／効果／用語集／精度の読み方）
- [データ理解](docs/data-understanding.md) — **このデータで何が言えて、何が言えないか**（数字を読む前に）
